...
class CreateAppContent extends Migration
{
    public function up(): void
    {
        ...
        // Seed default content — INSERT IGNORE is safe to re-run if rows already exist
        ...
        foreach ($seeds as $row) {
            $this->db->query(
                'INSERT IGNORE INTO app_content (content_key, title, content, created_at, updated_at) VALUES (?, ?, ?, ?, ?)',
                [$row['content_key'], $row['title'], $row['content'], $now, $now]
            );
        }
    }
